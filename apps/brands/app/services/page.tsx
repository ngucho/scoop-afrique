  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <section className="relative overflow-hidden border-b border-border bg-foreground py-20 text-background md:py-28">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-10 px-5 sm:px-8 md:px-12 lg:grid-cols-[1fr_0.82fr] lg:px-20">
          <div>
            <div className="flex items-center gap-2 font-mono text-xs uppercase tracking-widest text-primary">
              <Dot size="sm" className="text-primary" />
              Offres & prix 2026
            </div>
            <h1 className="mt-4 max-w-4xl text-4xl font-black leading-tight text-background md:text-6xl" style={{ fontFamily: 'var(--font-headline)' }}>
          ...
          <Link href={`/services/${featured.slug}`} className="group relative min-h-[430px] overflow-hidden rounded-2xl border border-background/12 bg-background/10 shadow-2xl">
            ...
            <div className="absolute inset-x-0 bottom-0 p-6 text-white md:p-8">
              ...
              <h2 className="max-w-md text-3xl font-black leading-tight md:text-4xl" style={{ fontFamily: 'var(--font-headline)' }}>
                {featured.title}
              </h2>
      ...
      <section className="border-b border-border bg-card py-10">
        <div className="mx-auto grid max-w-7xl gap-3 px-5 sm:px-8 md:grid-cols-3 md:px-12 lg:px-20">
          {['Prix visibles', 'Livrables detailles', 'Brief en 24-48h'].map((item) => (
            <div key={item} className="flex items-center gap-3 rounded-2xl border border-border bg-background p-4">
              ...
              <span className="font-mono text-xs uppercase tracking-widest text-muted-foreground">{item}</span>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-5 sm:px-8 md:px-12 lg:px-20">
          <div className="mb-10 grid gap-4 md:grid-cols-[0.7fr_0.3fr] md:items-end">
            <div>
              ...
              <h2 className="mt-3 text-3xl font-black leading-tight md:text-5xl" style={{ fontFamily: 'var(--font-headline)' }}>
