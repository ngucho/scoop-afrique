...
export async function listContracts(params?: {
  ...
  const searchWhere = q
    ? or(
        ilike(crmContracts.title, `%${q}%`),
        ilike(crmContracts.reference, `%${q}%`),
  ...
  const countQuery = useJoin
    ? db
        .select({ count: sql<number>`count(distinct ${crmContracts.id})::int` })
        .from(crmContracts)
  ...
  return {
    data: rows.map((r) =>
      toSnakeRecord(
        (useJoin ? (r as { contract: typeof crmContracts.$inferSelect }).contract : r) as Record<string, unknown>
      )
    ),
    total: count ?? 0,
  }
