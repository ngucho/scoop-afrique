...
export default async function ProjectsPage() {
  ...
  const stats = {
    active: activeProjects.filter((p) => ['active', 'in_progress'].includes(String(p.status))).length,
    completed: activeProjects.filter((p) => ['completed', 'closed'].includes(String(p.status))).length,
    totalBudget: activeProjects.reduce((sum, p) => sum + (Number(p.budget_agreed) ?? 0), 0),
  }
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {archivedProjects.map((p, i) => {
              ...
              const startDate = p.start_date
                ? new Date(p.start_date as string).toLocaleDateString('fr-FR', { month: 'short', year: 'numeric' })
                : null
              ...
              return (
                ...
                >
                  ...
                  {p.description != null && String(p.description) !== '' ? (
                    <p className="text-xs text-muted-foreground line-clamp-2 mb-3">{String(p.description)}</p>
                  ) : null}
