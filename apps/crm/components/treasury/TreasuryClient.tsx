...
export function TreasuryClient({
  ...
  function openEdit(m: Movement) {
    ...
    setForm({
      ...
      receipt_url: String(m.receipt_url ?? ''),
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            ...
            <Input
              ...
              className="mt-1"
              placeholder="https://… (scan, facture fournisseur, etc.)"
            />
            <p className="text-xs text-muted-foreground mt-1">Optionnel — lien vers une image ou un document.</p>
          </div>
          <div className="sm:col-span-2">
            <Label>Notes</Label>
            <Input
        ...
        {viewMode === 'cards' ? (
          <div className="p-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {initialMovements.map((m) => {
              ...
              const url = String((m as Movement).receipt_url ?? '').trim()
              return (
                <div key={String(m.id)} className="rounded-lg border border-border p-4 space-y-2 text-sm">
                  ...
                  <p className="text-xs text-muted-foreground">{catLabel(dir, String(m.category ?? ''))}</p>
                  <p className="font-medium line-clamp-2">{String(m.title ?? '—')}</p>
                  <p
                    className={`text-lg font-bold tabular-nums ${dir === 'income' ? 'text-emerald-700 dark:text-emerald-400' : 'text-red-700 dark:text-red-400'}`}
                  >
                    {dir === 'expense' ? '−' : '+'}
                    {formatMoney(amt)}
                  </p>
            ...
            <table className="crm-table">
              <thead>
                <tr>
                  ...
                  <th className="text-center w-20">Justif.</th>
