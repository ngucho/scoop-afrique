...
export interface AfricanPatternProps extends React.SVGAttributes<SVGSVGElement> {}

export function AfricanPattern({ className, ...props }: AfricanPatternProps) {
  ...
  useEffect(() => {
    ...
    paths?.forEach((path, index) => {
      path.setAttribute(
        'style',
        `animation: scoop-draw-line 2s ease-out forwards; animation-delay: ${index * 0.1}s; stroke-dasharray: 1000; stroke-dashoffset: 1000;`
      )
    })
  }, [])
